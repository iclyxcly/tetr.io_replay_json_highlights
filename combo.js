        ...
        try {
            ...
            for (let i = 0; i < replayData.data.length; ++i) {
                ...
                if (result[result.length - 1].type === "end" && result[result.length - 1].data.export.stats.topcombo - 1 >= comboThresold)
                    outputStr += "Round " + (i + 1) + (i + 1 < 10 ? "  " : i + 1 < 100 ? " " : "") + (result[result.length - 1].data.reason === "winner" ? " (W)" : " (L)") + ": " + (result[result.length - 1].data.export.stats.topcombo - 1) + " combo" + "\n", ++ foundCnt;
            }
            if (outputStr !== "")
                console.log(file + ": \n" + outputStr + "-----------------------------");
        } catch (err) {
...
console.log("Total occurrence for " + comboThresold + " combo++: " + foundCnt);